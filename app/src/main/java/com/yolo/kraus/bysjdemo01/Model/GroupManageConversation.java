...
import com.yolo.kraus.bysjdemo01.Activity.GroupManageMessageActivity;
...
public class GroupManageConversation extends Conversation {
    ...
    public void readAllMessage() {
        ...
        GroupManagerLogic.readGroupManageMessage(Calendar.getInstance().getTimeInMillis(), new TIMCallBack() {
            @Override
    ...
    public String getLastMessageSummary() {
        ...
        boolean isSelf = from.equals(nowInfo.getInstance().getId());
        switch (lastMessage.getPendencyType()){
            case INVITED_BY_OTHER:
                if (isSelf){
                    ...
                    if (to.equals(nowInfo.getInstance().getId())){
                        return from+
                                ImApplication.getContext().getResources().getString(R.string.summary_group_invite)+
